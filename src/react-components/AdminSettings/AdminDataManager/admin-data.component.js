...
export default class AdminData extends Component{
    ...
    familyMode(){ this.setState({dataType: "family"})}
    ...
    */
    handleSubmit(e){
        e.preventDefault()
        if(this.state.dataType === "family" && this.state.dataType.length > 0){
            ...
            else alert("This family name is not valid (it might be too short or already exists)")
            ...
            else alert("A store with that address already exists (or it's too short)")
            ...
            else alert("A tribe with that name already exists (or it's too short)")
        }
    }
